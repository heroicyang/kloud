...
/**
 * Delete up to 1000 files at a time, with optional `headers` and `callback`.
 *
 * @param {Array[String]}   filenames
 * @param {Object}   headers
 * @param {Function} callback  Callback function with (err, res)
 */
proto.deleteMultiple = function(filenames, headers, callback) {
  if (filenames.length > 1000) {
    throw new Error('Can only delete up to 1000 files at a time. ' +
      'You\'ll need to batch them.');
  }
  ...
  var tags = filenames.map(function(filename) {
    return '<Object><Key>' +
      filename
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
  ...
  var buf = new Buffer('<?xml version="1.0" encoding="UTF-8"?>' +
      '<Delete>' + tags.join('') + '</Delete>', 'utf8');

  headers['Content-Length'] = buf.length;
  headers['Content-MD5'] = utils.md5(buf);
